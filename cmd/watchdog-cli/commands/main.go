...
func init() {
	...
	rootCommand.PersistentFlags().String("server-url", "", "watchdog server url")
	rootCommand.PersistentFlags().String("server-token", "", "watchdog server token")
	...
	rootCommand.PersistentFlags().StringP("policies-file", "p", "", "path to external .watchdog.yml file")
